...
<strong>Замечание:</strong> Скорее всего на этом этапе большая часть команд для вас просто бессмыслица и вы задаетесь вопросами: "Что тут происходит? Зачем все это надо? Так надо будет делать каждый раз?". Я вас понимаю и это нормально (^_^). Вот пара советов: 1) ходите по ссылкам, которые я даю и внимательно читайте; 2) если остались вопросы, то погуглите или задайте вопрос в Slack'е; 3) перепечатывайте все команды и код вручную, не копируйте, так проще запомнить; 4) если что-то не получается, то не паникуйте и не бросайте то, что вы делаете, обратитесь к пункту №2.
</div>
