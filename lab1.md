...
Каждое задание оценивается в отдельности, таким образом, оценку вы получаете по совокупности выполненных заданий (в лабораторной работе №1 таких заданий - три). В оценку включается балл за стиль выполнения задания, поэтому прочитайте официальное руководство по стилю https://www.python.org/dev/peps/pep-0008/ (также некоторые пояснения по стилю можно найти [здесь](https://github.com/Dementiy/pydonts)). Соответствие вашей работы PEP8 можно узнать "прогнав" исходный код вашей программы через утилиту `pep8`.

Все исходники к работам вы можете найти в [репозитории курса](https://bitbucket.org/Dementiy/cs102_2016) (следите за его обновлениями). Чтобы получить список всех работ достаточно выполнить следующую команду:
...
Все дальнешие изменения в репозитории можно получать с помощью команды `git pull` (ее рекомендуется выполнять каждый раз, когда вы приступаете к выпонению новой лабораторной работы):
...
По завершению работы над каждой функцией не забудьте также сделать коммит, например:
...
<div class="alert alert-info">
<strong>Замечание</strong>: Вы можете воспользоваться приложением <a href="https://www.sourcetreeapp.com/">Source Tree</a>, чтобы более наглядно отслеживать все изменения, которые вы вносите.
</div>

<div class="alert alert-info">
<strong>Практика</strong>: Попробуйте изменить ваши функции так, чтобы размер сдвига был произвольным:<br/>
...
По окончанию работы над каждой функцией не забудьте сделать соответствующие коммиты как это было в примере с шифром Цезаря, а также коммит для объединения изменений с веткой `develop`.
...
Теперь можете начать работать над функцией `is_prime(n)`:
...
Не забудьте закоммитеть реализацию функции `gcd(a, b)`:
...
Когда вы выполните все задачи из лабораторной работы, то создайте новый релиз:
