...
В этой лабораторной работе вашей задачей является написание симулятора игры «Жизнь».
...
- Существо с двумя или тремя соседями выживает в следующем поколении, иначе оно погибает от одиночества или перенаселённости.
- В пустой клетке с тремя соседями в следующем поколении рождается существо.

[Программа для моделирования игры «Жизнь»](https://life.written.ru).
...
Далее представлен листинг программы, позволяющей создать игровое поле:
...
Теперь нам нужно окрашивать клетки в зависимости от их состояния. Каждая клетка на поле представлена прямоугольником размера `cell_size` $$\times$$ `cell_size`, который можно закрасить с помощью функции `pygame.draw.rect`. Эта функция принимает следующие параметры:

* `Surface` - поверхность для отрисовки прямоугольника (в нашем случае это `screen`);
...
Теперь осталось написать метод для обновления поля, в котором состояния клеток будут меняться по описанным в начале правилам. Чтобы определить, как должно измениться состояние клетки, необходимо получить состояние её соседей. Напишите функцию для получения списка соседей клетки:
...
Важно помнить, что обновление всего поля должно происходить одновременно. Если состояние клеток менять последовательно, то это повлияет на результат игры.
</div>

Теперь у вас должна быть полностью рабочая игра.

Изменим дизайн нашей игры. Вы могли заметить, что клетка это объект, который имеет атрибут (поле) - состояние; список клеток также является объектом, над которым можно выполнять такие действия, как заполнение и обновление. Вам необходимо реализовать классы `Cell` и `CellList`, которые представляют собой клетку и список ячеек (список объектов типа `Cell`). Ниже приведен пример интерфейса (возможных методов):
...
Класс `CellList` должен быть итерируемым (соответствовать iterator protocol). 
Например, так можно пройтись по списку клеток и оживить случайные клетки:
